...
| `ABUSEIPDB_MAX_REQUESTS_PER_SOURCE_IP_PER_MINUTE` | `0` | Ergänzende Begrenzung pro einzelner Quell-IP (gleitendes 60s-Fenster), statt der Gesamtsumme über alle Quellen wie oben. Vor allem relevant bei `ABUSEIPDB_LISTEN_ADDRESS=0.0.0.0`-Setups, wo der Endpunkt nicht zwingend nur von einer vertrauenswürdigen CrowdSec-Instanz auf localhost erreichbar ist. `0` (Standard) deaktiviert es. Lehnt bei Überschreitung mit `429` + `Retry-After: 60` ab. |
...
| `ABUSEIPDB_QUOTA_RESERVE_RECHECK_DELAY` | `300` | Wie lange (Sekunden) ein durch obige Quota-Reservierung zurückgehaltener Report wartet, bevor das Restkontingent erneut geprüft wird. Der Report wird dabei nur verschoben, nicht verworfen – ist das Kontingent bei der erneuten Prüfung immer noch reserviert, wird er erneut verschoben. |
...
| `ABUSEIPDB_LOG_HOSTNAME` | (nicht gesetzt) | Optionales Hostname-/Instanz-Tag, das jeder `json`-formatierten Log-Zeile hinzugefügt wird (ohne Wirkung im `text`-Format). Standardmäßig aus – bei einem Ein-Instanz-Setup nutzlos. Sinnvoll, sobald dieser Proxy an mehr als einem Standort läuft und json-Ausgaben an einen gemeinsamen Log-Aggregator geschickt werden, wo sich sonst nicht unterscheiden lässt, welche Instanz welche Zeile geschrieben hat. `true`/`1`/`yes` erkennt den Systemhostnamen automatisch; jeder andere nicht-leere Wert wird wörtlich übernommen (z. B. ein sprechenderer Standortname als der eigentliche Hostname). |
| `ABUSEIPDB_QUOTA_WARN_THRESHOLD` | `50` | Schickt einmal pro Tag eine Benachrichtigung (über einen konfigurierten Alarm-Mechanismus), sobald das tägliche AbuseIPDB-Report-Kontingent auf diesen Wert oder darunter fällt. Wird aus dem `X-RateLimit-Remaining`-Header der API mitgelesen – auch über `/health` und `/metrics` sichtbar. Sobald genug vom heutigen Verbrauchsverlauf bekannt ist, enthält die Benachrichtigung auch eine voraussichtliche Erschöpfungszeit (auch in `--doctor`/`--stats` sichtbar). |
...
| `ABUSEIPDB_ORPHAN_RESCAN_INTERVAL` | `60` | Sekunden zwischen erneuten Prüfungen auf persistierte Pending-/Retry-Zeilen ohne passenden In-Memory-Timer in diesem Prozess – wird bei Fund neu bewaffnet. Vor allem relevant für `--reconcile`: Ein dort ausgelöster Report, der einen Retry braucht, hat nur in diesem kurzlebigen CLI-Prozess einen Timer, der mit dessen Beendigung verschwindet – das hier ist, was den Retry im dauerhaft laufenden Dienst tatsächlich wieder aufgreift, statt dass er bis zum nächsten Neustart dieses Dienstes stillschweigend im Cache liegen bleibt. `0` deaktiviert es. |
| `ABUSEIPDB_RETRY_BACKLOG_WARN_SIZE` / `_AGE` | `20` / `900` | Schickt einmal pro Episode eine Benachrichtigung (über einen konfigurierten Alarm-Mechanismus), wenn der kombinierte Pending-/Retry-Rückstau durchgehend mindestens `_AGE` Sekunden lang bei `_SIZE` oder darüber bleibt. Eine reine Anzahlprüfung wäre zu unruhig – Dutzende ausstehende Eskalationen gleichzeitig sind auf einer stark ausgelasteten Instanz normal – deshalb löst dies erst aus, wenn diese Anzahl anhaltend hoch bleibt statt sich wie üblich abzubauen, was meist darauf hindeutet, dass AbuseIPDBs API selbst nicht erreichbar ist oder Fehler wirft, statt eines Problems mit einer einzelnen IP. Setzt sich zurück, sobald der Rückstau wieder unter `_SIZE` fällt, sodass eine spätere, eigenständige Episode eine eigene frische Warnung bekommt. `_SIZE=0` deaktiviert es. |
...
`abuseipdb-proxy.service` nutzt `Type=notify`: Der Proxy meldet systemd genau dann, wenn er tatsächlich bereit ist (nachdem alle noch offenen Eskalationen/Retries aus einem vorherigen Lauf vollständig wiederaufgesetzt wurden – siehe `resume_state_from_cache()` weiter unten – nicht schon, sobald der Prozess gestartet ist), und meldet bei `systemctl stop`/SIGTERM sauber `STOPPING=1`, statt einfach getötet zu werden. Das bedeutet, `systemctl start abuseipdb-proxy` blockiert tatsächlich, bis der Proxy bereit ist, Alerts entgegenzunehmen (nützlich in Skripten/Automatisierungen, die den Dienst starten und sofort erwarten, dass er lauscht), und `Restart=always` löst bei einem normalen, gewollten Stop nicht fälschlich aus. Das erfordert keinerlei Konfiguration – es funktioniert automatisch unter der mitgelieferten Unit-Datei und ist überall sonst (Docker, manueller Aufruf des Skripts) ein harmloses No-op.
...
| `--stats [--json] [--stats-limit N]` | Cache-Snapshot: aktuelle Reports, ausstehende Eskalationen, wartende Retries, AbuseIPDB-Kontingent (inkl. voraussichtlicher Erschöpfungszeit, sobald genug vom heutigen Verbrauch bekannt ist). `--json` fürs Scripting; `--stats-limit` begrenzt die Recent-Reports-Liste (Standard 10). |
| `--simulate KATEGORIEN [--simulate-comment TEXT] [--json]` | Zeigt vorab, wie eine kommagetrennte Kategorien-Zeichenkette (z. B. `15,18`, genau das, was CrowdSecs HTTP-Notification-Plugin sendet) behandelt würde: abgeleitete Severity, welches Report-Fenster greift und warum (Severity-Standard vs. Kategorie-Override), und ob es aktuell durch eine Quota-Reservierung zurückgehalten würde. Schickt nichts und braucht keine echte IP – zum Testen einer Severity-/Fenster-/Quota-Reserve-Konfigänderung, bevor sie live geht. Mit `--simulate-comment` zusätzlich eine Vorschau, wie `ABUSEIPDB_COMMENT_SCRUB_PATTERNS` auf einen Beispielkommentar wirkt. |
...
- **`GET /health`** – JSON-Status: `{"status", "version", "dry_run", "uptime_seconds", "cache_reports_tracked", "pending_escalations", "pending_retries", "oldest_pending_escalation_age_seconds", "oldest_pending_retry_age_seconds", "abuseipdb_quota"}`. Die beiden `oldest_*_age_seconds`-Felder sind `null`, wenn die jeweilige Queue leer ist oder der älteste Eintrag noch aus der Zeit vor der SQLite-Migration stammt, die dieses Tracking eingeführt hat – ansonsten geben sie an, wie viele Sekunden die älteste noch wartende Eskalation/der älteste Retry bereits durchgehend in der Queue steht. Das ist ein deutlich aussagekräftigeres Signal fürs Alerting als die reine Anzahl allein (eine Queue mit 5 Einträgen, die sich normal auf- und abbaut, sieht ganz anders aus als eine, in der dieselben 5 seit einer Stunde feststecken).
- **`GET /metrics`** – Prometheus-Textformat: `abuseipdb_proxy_reports_sent_total`, `abuseipdb_proxy_reports_suppressed_total`, `abuseipdb_proxy_reports_failed_total`, `abuseipdb_proxy_reports_ignored_private_total`, `abuseipdb_proxy_reports_quota_reserved_total`, `abuseipdb_proxy_reports_whitelisted_total`, `abuseipdb_proxy_reports_rejected_overload_total`, `abuseipdb_proxy_reports_rejected_rate_limited_total`, ein `abuseipdb_proxy_report_latency_seconds`-Histogram (wie lange jeder tatsächliche AbuseIPDB-API-Call dauert – nicht im Dry-Run erfasst – nützlich um „AbuseIPDB ist gerade langsam" von „AbuseIPDB ist down" zu unterscheiden), plus Gauges für ausstehende Eskalationen/Retries, Uptime und AbuseIPDB-Kontingent (Rest/Limit, sobald bekannt)
...
Da `--reconcile` als eigener kurzlebiger Prozess läuft, kann ein dort ausgelöster Report, der einen Retry braucht, nicht selbst auf diesen Retry warten – der Prozess beendet sich, sobald der Lauf fertig ist. Stattdessen greift der dauerhaft laufende Proxy-Dienst diesen Retry über seine periodische `ABUSEIPDB_ORPHAN_RESCAN_INTERVAL`-Prüfung (Standard alle 60s) wieder auf – derselbe Mechanismus, der auch bei einem normalen Neustart offene Arbeit neu bewaffnet, hier nur periodisch wiederholt, damit auch von einem anderen Prozess nach dem Start geschriebene Zeilen erfasst werden.
