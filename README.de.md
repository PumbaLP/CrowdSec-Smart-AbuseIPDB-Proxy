...
| `ABUSEIPDB_IGNORE_PRIVATE` | `true` | Bei `true` werden RFC1918-/Loopback-/Link-Local-/CGNAT-Adressen stillschweigend übersprungen (nie sinnvoll zu melden). Deckt auch Tailscales 100.64.0.0/10-Bereich ab. |
...
| `--doctor [--no-network] [--json]` | Alles, was `--check-config` prüft, plus systemd-Service-Status, Dateiberechtigungen, CrowdSec-`profiles.yaml`-Verdrahtung, Cache-Lesbarkeit, und (außer bei `--no-network`) ob api.abuseipdb.com erreichbar ist. Bare-Metal-spezifische Checks überspringen sich sauber außerhalb dieses Kontexts (z. B. in Docker). |
