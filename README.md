...
| `ABUSEIPDB_MAX_REQUESTS_PER_SOURCE_IP_PER_MINUTE` | `0` | Complementary per-source-ip cap (rolling 60s window), rather than the total across every source combined like the one above. Mainly relevant for `ABUSEIPDB_LISTEN_ADDRESS=0.0.0.0` deployments, where the endpoint isn't necessarily only reachable from a trusted CrowdSec instance on localhost. `0` (default) disables it. Rejects with `429` + `Retry-After: 60` once a single source exceeds the limit. |
...
| `ABUSEIPDB_LOG_HOSTNAME` | (unset) | Optional hostname/instance tag added to every `json`-format log line (no effect on `text` format). Off by default — a single-instance deployment has no use for it. Useful once you're running this proxy at more than one site and shipping json output to a shared log aggregator, where otherwise there's no way to tell which instance wrote which line. `true`/`1`/`yes` auto-detects via the system hostname; any other non-empty value is used verbatim (e.g. a friendlier site name than the actual hostname). |
...
- **`GET /metrics`** — Prometheus text format: `abuseipdb_proxy_reports_sent_total`, `abuseipdb_proxy_reports_suppressed_total`, `abuseipdb_proxy_reports_failed_total`, `abuseipdb_proxy_reports_ignored_private_total`, `abuseipdb_proxy_reports_quota_reserved_total`, `abuseipdb_proxy_reports_whitelisted_total`, `abuseipdb_proxy_reports_rejected_overload_total`, `abuseipdb_proxy_reports_rejected_rate_limited_total`, a `abuseipdb_proxy_report_latency_seconds` histogram (how long each actual AbuseIPDB API call takes — not recorded in dry-run — useful for telling "AbuseIPDB is slow right now" apart from "AbuseIPDB is down"), plus gauges for pending escalations/retries, uptime, and AbuseIPDB quota remaining/limit (once known)
