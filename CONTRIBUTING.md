...
**Checking coverage:** `pytest --cov=. --cov-report=term-missing` (CI runs the same command). Use `--cov=.`, not `--cov=abuseipdb_proxy` — the latter silently reports "No data to report" for this project specifically, since `conftest.py` gives every test a freshly re-imported copy of the module under a synthetic name (see the module docstring at the top of `conftest.py`) rather than importing it once under its real name, which is what `--cov=abuseipdb_proxy`'s name-based resolution depends on; `--cov=.` is path-based and unaffected. CLI-only code (`parse_args()`, the `__main__` dispatch block) is measured too, via a `sitecustomize.py` at the repo root plus a `pytest_configure` hook in `conftest.py` that together extend coverage into the real subprocesses `test_cli.py`/`test_check_config.py`/etc. spawn — automatic, no extra flags needed, and a complete no-op for a plain `pytest` run with no `--cov`.
